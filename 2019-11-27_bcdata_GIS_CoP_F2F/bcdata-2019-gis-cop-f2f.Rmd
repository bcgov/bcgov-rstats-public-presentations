...
- **`bcdc_get_data()`** 
  - Get catalogue data
- **`bcdc_query_geodata()`** 
  - Get & query B.C. geospatial data available through a [Web Service](https://www2.gov.bc.ca/gov/content?id=95D78D544B244F34B89223EF06DF74E)
]
...
mapview(crd_mun, zcol = "ADMIN_AREA_ABBREVIATION")
```
...
ggplot(crd_mun) + 
  geom_sf(aes(fill = as.numeric(percent_green))) + 
